...
def list_games():
    ...
    games_data = [
        {"id": g.id, "title": g.title, "year": g.year, "price": g.price}
        for g in games
    ]
...
def get_game(game_id):
    ...
    return jsonify({"id": game.id, "title": game.title,
                    "year": game.year, "price": game.price}), 200
...
def bad_request(e): return jsonify({"error": "Requisição inválida"}), 400
@app.errorhandler(401)
def unauthorized(e): return jsonify({"error": "Não autorizado"}), 401
@app.errorhandler(403)
def forbidden(e): return jsonify({"error": "Proibido"}), 403
@app.errorhandler(404)
def not_found(e): return jsonify({"error": "Recurso não encontrado"}), 404
@app.errorhandler(405)
def method_not_allowed(e): return jsonify({"error": "Método não permitido"}), 405
@app.errorhandler(413)
def too_large(e): return jsonify({"error": "Requisição muito grande. Limite = 1 MB"}), 413
@app.errorhandler(500)
def internal(e):
    app.logger.error(f"Erro interno: {e}")
    return jsonify({"error": "Erro interno do servidor"}), 500
@app.errorhandler(ValidationError)
def validation(e): return jsonify({"errors": e.messages}), 400
...
if __name__ == '__main__':
    with app.app_context():
        ...
        if not User.query.filter_by(email='diego@email.com').first():
            db.session.add(User(name='Diego',
